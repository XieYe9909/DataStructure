...
BiTree CreateTree(int a[],int len){
	if(len==0) return NULL;
		...
		int n=num(a,len);
		BiTree p=TreeNode(*a);
		p->lchild=CreateTree(a+1,n);
		p->rchild=CreateTree(a+n+1,len-n-1);
		return p;
	}
}
int PostOrder(BiTree T,int &i,int lev){
	int nl=0,nr=0;
	if(!T) return 0;
	else{
		lev++;
		...
		nl=PostOrder(T->lchild,i,lev);
		nr=PostOrder(T->rchild,i,lev);
		printf("%d ",T->data);
		bmm[T->data-1][3]=lev;
